...
create or replace function public.register_user(p_username text, p_password text, p_public_key text)
returns uuid
...
begin
  ...
  insert into public.access_logs (user_id, event, user_agent) values (target_id, 'login', current_setting('request.headers', true));
...
create or replace function public.get_group_message(p_id uuid)
